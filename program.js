...
//end electron mainjs code



//start craigslist scraper code

//chase edson april 16th, 2021 in san jose california
//can't sleep so we gonna rewrite this scraper shit in node
//mainly because i want to learn node and maybe build a desktop app with electron or something i dunno
//if anyone is looking at this it is probably terrible code so i'm sorry


//needs node-fetch and jssoup

//oh and also this static list of craigslist URLS SORRY HAHAHAHA
...
class Listing {
    constructor(url, title, price, image_urls) {
        this.url = url;
        this.title = title;
        this.price = price;
...
var listings_info = [];
var total_listing_count;

//return inputted data list without diplicates


//just parsing here.. no web requests
//finds the listings to individual URLS on each of the main pages
async function findListingHREF(data) {
    var soup = new JSSoup(data);
    var found = soup.findAll('a', 'result-title');
...
//parse the listing information from the individual listing page...
async function getListingData(data, url) {
    var soup = new JSSoup(data);

    var title = soup.find("span", { "id": "titletextonly" }).getText();

    var tempprice = soup.find("span", { "class": "price" }).getText();
    var price = tempprice.substring(1);

    var imagesfull = soup.findAll('img');
    var imageurls = [];
...
async function findListings(query) {
    ...
    for (let index = 0; index < urlContainer1.length; index++) {
        ...
        try {
            ...
            const response = await fetch(currentURL).then(res => res.text()).then(body => findListingHREF(body));;
    ...
    return findListingInfo(listings);


}
//for the full list of urls, this function goes to each one and sends it to getListingData
async function findListingInfo(listings) {
    for (let index = 0; index < listings.length; index++) {

        const currentURL = listings[index];

        try {

            const response = await fetch(currentURL).then(res => res.text()).then(body => getListingData(body, currentURL));;
            console.log("REQUEST SUCCESSFUL.");
...
//end craigslist scraper code



// example query string: /search/cto?query=tesla+model+3&purveyor-input=owner&min_price=25000&max_price=40000&min_auto_miles=1000&max_auto_miles=45000&auto_title_status=1



ipcMain.on('asynchronous-message', async (event, arg) => {
  //reset if multiple runs performed
  listings = [];
  listings_info = [];
  console.log("query string received: " + arg);
  let foundlistings = await findListings(arg);
  event.reply('asynchronous-reply', foundlistings)
